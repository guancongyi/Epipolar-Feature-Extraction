...
class rectification():
    def __init__(self, cx, cy, f, R, miu):
        ...
        self.miu = miu
        # 1
        # self.a1, self.a2, self.a3 = 0.8546988,  0.5184653, -0.0261467 #R[0, 0], R[0, 1], R[0, 2]
        # self.b1, self.b2, self.b3 = -0.5190966,  0.8540881, -0.0327441#R[1, 0], R[1, 1], R[1, 2]
        # self.c1, self.c2, self.c3 = 0.0053550,  0.0415590,  0.9991217#R[2, 0], R[2, 1], R[2, 2]
        # 2
        # self.a1, self.a2, self.a3 = -0.7088435, -0.7051392,  0.0178782  # R[0, 0], R[0, 1], R[0, 2]
        # self.b1, self.b2, self.b3 = 0.7053359, -0.7083523,  0.0271739  # R[1, 0], R[1, 1], R[1, 2]
        # self.c1, self.c2, self.c3 = -0.0064973,  0.0318722,  0.9994708  # R[2, 0], R[2, 1], R[2, 2]
        # 3
        self.a1, self.a2, self.a3 = 0.8540546,  0.5200711,  0.0108056  # R[0, 0], R[0, 1], R[0, 2]
        self.b1, self.b2, self.b3 = -0.5199969,  0.8541204, -0.0090335  # R[1, 0], R[1, 1], R[1, 2]
        self.c1, self.c2, self.c3 = -0.0139273,  0.0020962,  0.9999008  # R[2, 0], R[2, 1], R[2, 2]
        R
